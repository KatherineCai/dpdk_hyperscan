...
RunMainLoop(struct mtcp_thread_context *ctx)
{
	...
	//add by songhui
	if(ctx->cpu == 0){
		high_core0 = (struct pro_buffer*)calloc(1, sizeof(struct pro_buffer));
		low_core1 = (struct pro_buffer*)calloc(1, sizeof(struct pro_buffer));
		high_core0->pktbuf_que = (struct pkt_data*)calloc(MAX_HIGH, sizeof(struct pkt_data));
		low_core1->pktbuf_que = (struct pkt_data*)calloc(MAX_HIGH, sizeof(struct pkt_data));
		high_core0->head = 0;
		high_core0->tail = 0;
		low_core1->head = 0;
		low_core1->tail = 0;
	...
	TRACE_INFO("pro_buffer %d bytes,pkt_data %d bytes.\n",sizeof(struct pro_buffer),sizeof(struct pkt_data));	
	...
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
		...
		for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
			...
			for (i = 0; i < recv_cnt; i++) {				
				...
				if(ctx->cpu == 0){
					if(high_core0->head != (high_core0->tail + 1) % MAX_HIGH){
						high_core0->pktbuf_que[high_core0->tail].pktbuf = pktbuf;
						high_core0->pktbuf_que[high_core0->tail].len = len;
						high_core0->tail = (high_core0->tail + 1) % MAX_HIGH;
					}else
						perror("high_core0 already full!");					
				}else{
					if(high_core0->head != high_core0->tail){						
						if(low_core1->head != (low_core1->tail + 1) % MAX_HIGH){
							low_core1->pktbuf_que[low_core1->tail].pktbuf = pktbuf;
							low_core1->pktbuf_que[low_core1->tail].len = len;
							low_core1->tail = (low_core1->tail + 1) % MAX_HIGH;
						}else
							perror("low_core1 already full!");
					}else
			...
			if(ctx->cpu == 1){
				while(high_core0->head != high_core0->tail){
					pktbuf = high_core0->pktbuf_que[high_core0->head].pktbuf;
					len = high_core0->pktbuf_que[high_core0->head].len;
					//TRACE_INFO("high_core0->head : %d,high_core0->tail : %d\n", high_core0->head,high_core0->tail);
					//TRACE_INFO("pktbuf : %p,len : %d\n", pktbuf,len);
					ProcessPacket(mtcp, rx_inf, ts, pktbuf, len);
					high_core0->head = (high_core0->head + 1) % MAX_HIGH;
				}
				 while(low_core1->head != low_core1->tail){
                                        pktbuf = low_core1->pktbuf_que[low_core1->head].pktbuf;
					len = low_core1->pktbuf_que[low_core1->head].len;
                                        ProcessPacket(mtcp, rx_inf, ts, pktbuf, len);
                                        low_core1->head = (low_core1->head + 1) % MAX_HIGH;
					if(high_core0->head != high_core0->tail){
						 pktbuf = high_core0->pktbuf_que[high_core0->head].pktbuf;
                                        	 len = high_core0->pktbuf_que[high_core0->head].len;
                                                 ProcessPacket(mtcp, rx_inf, ts, pktbuf, len);
