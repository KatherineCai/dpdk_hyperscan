 ...
 *
 *                 dpdk_check_device :add function for qstack
 *
 *                 dpdk_rx_receive:add function for qstack
 *                 return -1 on failure and return receive packets' number
 *
 *                 dpdk_tx_send:add funciton for qstack send packet,return -1 on failure or return 
 *		   send packets' number
 */
typedef struct io_module_func {
	void	  (*load_module)(int queue_num);
	...
	int32_t	  (*dev_ioctl)(struct mtcp_thread_context *ctx, int nif, int cmd, void *argp);
        ...
        void      (*check_ringbuffer)(struct mtcp_thread_context *ctx);
        int32_t   (*dev_rx_receive)(struct mtcp_thread_context *ctx,int level,int port,struct rte_mbuf **ptr);
        unsigned char  *(*dev_rx_receive_one)(struct mtcp_thread_context *ctx,int level,int port);
        int32_t   (*dev_tx_send)(struct mtcp_thread_context *ctx,int level,int port,struct rte_mbuf **ptr,int len);
        uint8_t  *(*dev_alloc_mbuf)(struct mtcp_thread_context *ctx, int ifidx);
        void      (*check_rxring)(struct mtcp_thread_context *ctx);
        void      (*check_txring)(struct mtcp_thread_context *ctx);
        int32_t   (*check_rx_state)(struct mtcp_thread_context *ctx,int level,int port);
