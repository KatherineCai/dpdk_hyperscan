...
static struct rte_mempool *pktmbuf_pool[16 ] = {NULL};
...
int dpdk_soft_filter(struct rte_mbuf * buf){
...
//			       dpc->pkts_burst, MAX_PKT_BURST);
    ...
    printf("ret is %d in rx and cpu id is %d \n",ret,ctxt->queue);
    }
    ...
    for( i = 0	;i< ret ;i++){
         ...
         head = rx_mbufs->head;
         tail = rx_mbufs->tail;
//         printf("in check tail is %d in rx queue id is  %d\n",tail,ctxt->queue);
         printf("in check head is %d in rx queue id is %d \n",head,ctxt->queue);
...
//second for send
#if 1
    struct mbuf_table_q *tx_mbufs = &dpc->tl_mbufs[portid];

    struct mbuf_table_q *tx_low_mbufs = &dpc->th_mbufs[portid];
    ...
    int count = 0;
    if(max_send != 0){
 //        printf("we will  do something here and max_send is %d \n",max_send);
    }
    ...
    for(i = 0;i<max_send;i++){
       ...
       // printf("we will  do something here and send_num is %d and head is %d\n",send_num,tx_mbufs->head);
 
       // printf("we will  do something here and send_num is %d and tail is %d\n",send_num,tx_mbufs->tail);
      for(k = 0;k<send_num;k++){
           dpc->pkts_burst[k] = tx_mbufs->m_table[(head+i + k)%MAX_MTABLE];
           
       }

       // printf("portid is %d,ctxt->queue %d dpc->pkts_burst 0x%x send_num is %d\n",portid,ctxt->queue,dpc->pkts_burst,send_num);
       rte_eth_tx_burst((uint8_t)portid, (ctxt->queue),dpc->pkts_burst, send_num);
       i = i + send_num;
    }
    tx_mbufs->head = (head + max_send)%MAX_MTABLE;
#endif
}
int32_t  dpdk_rx_receive(struct mtcp_thread_context *ctxt,int level,int port,struct rte_mbuf **ptr){
    ...
    tail = rx_mbufs->tail;
//    if(ctxt->queue == 0){
//    printf("for test we run here and ctxt->queue is %d \n",ctxt->queue);
//    printf("in receive head is %d in this test \n",head);
    printf("in receive tail is %d in this test \n",tail);
//    }
    int be = tail - head;
    if(be < 0 ){
    be = be + MAX_MTABLE;
    ...
    for( i = 0; i < max_receive; i++){
        dpc->pkts_rxburst[i] = rx_mbufs->m_table[head + i ];
        start[i] = rx_mbufs->m_table[head + i ];
        rx_mbufs->head++;
        rx_num++;
    }  
...
int32_t   dpdk_tx_send(struct mtcp_thread_context *ctxt,int level,int port,struct rte_mbuf **ptr,int len){
    ...
    struct rte_mbuf **start = ptr;
    for( i = 0; i < max_send; i++){
        tx_mbufs->m_table[(tail + i)%MAX_MTABLE] = start[i];

    } 
 
    tx_mbufs->tail = (tail + max_send)%MAX_MTABLE;
    ret = max_send;
    dpc->tx_num = ret;
