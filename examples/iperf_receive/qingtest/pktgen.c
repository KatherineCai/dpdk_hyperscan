...
void build_ip_head_data(struct rte_mbuf *mbuf,int size,int id,int ip_type){
    ...
    ip_head->src_addr = swab32(IPv4(192, 168, 127, 88));
    ip_head->dst_addr = swab32(IPv4(192, 168, 127, 101));
...
void change_sdata(struct rte_mbuf *mbuf){
    ...
    tcp_head->src_port= 0x0081;
    tcp_head->dst_port = 0x0088;
}
...
int build_tcp_pkt(struct rte_mbuf *pkt_mbuf, int size, int seq_cnt){
     ...
     for(i = 0; i<size;i++){
          *(ptr + i) = i;
          ...
          int time = 0;
          time = seq_cnt % 2;
          if(i == (sizeof(struct ether_hdr)+sizeof(struct ip_head_data)+sizeof(struct tcphdr) + 6)){
          *(ptr + i) = 2 + time;
...
//          printf("mark is  %d \n", *(ptr + i));
  ...
  //   printf("printf sqe_cntis %d \n",seq_cnt);
     if(seq_cnt%2 == 1){
         change_sdata(pkt_mbuf);
     }
